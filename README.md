...
일일보고의 분석 검토 후보에는 종목별 투표가 붙는다. `분석 완료`, `30일 보류`, `다시 검토` 중 하나를 선택하면 기존 관리 기록에 반영된다. 결과는 웹훅이 보낸 메시지를 다시 조회해 읽으므로 추가 봇 계정이나 토큰이 필요하지 않다.

- `분석 완료`: 투표에 연결된 사건 시점까지 완료 처리한다. 투표를 늦게 눌러도 그 이후 사건까지 완료한 것으로 처리하지 않는다.
- `30일 보류`: 처음 선택을 확인한 시각부터 30일이다. 매번 확인할 때 기간을 연장하지 않는다.
- `다시 검토`: 수동 완료·보류 기록을 해제한다. 자동 분석이나 자동 매매를 실행하지 않는다.
- 다른 답으로 바꾸면 다음 확인에 반영한다. 선택 취소만으로 기존 적용 기록을 되돌리지는 않는다.
- 한 명이 쓰는 채널을 전제로 한다. 복수 표·충돌·결과 누락은 상태를 바꾸지 않는다. 최신 투표와 충돌하는 이전 투표도 적용하지 않는다.
- 투표는 32일 뒤 만료된다. 일일보고 전과 매시 7분·37분 예약 실행에서 확인한다. GitHub Actions 지연 때문에 실제 반영이 늦어질 수 있다.
- 투표 생성 시 반환되는 메시지 ID와 실제 answer ID를 영속 저장한다. 같은 이벤트에는 투표를 다시 보내지 않는다.

발송 없는 결과 확인: `python hub.py --polls-only`. 연결 테스트: `python hub.py --test-poll` (테스트 투표 1개만 생성하고 실제 종목 상태는 바꾸지 않는다).
